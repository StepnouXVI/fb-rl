...
def test_flax_gated_attention_translator_forward():
    model = build_flax_translator(model_type="gated_attn", latent_dim=128, hidden_dim=64, n_layers=2)
...
def test_flax_dense_eca_translator_forward():
    model = build_flax_translator(model_type="dense_eca", latent_dim=128, hidden_dim=64, n_layers=2)
    rng = jax.random.PRNGKey(42)
    dummy_s = jnp.ones((4, 29))
    dummy_g = jnp.ones((4, 128))
...
def test_differentiable_physics_training_step(setup_agent):
    ...
    step_fn = make_train_step(model, agent, optimizer)
...
def test_distilled_jax_planner(setup_agent):
    agent, env, train_ds, _, _ = setup_agent
    planner = DistilledJAXPlanner(agent, model_type="gated_attn", hidden_dim=64, n_layers=2)

    obs = train_ds["observations"][0]
    goal_z = agent.normalize_z(agent.network.select("backward_repr")(train_ds["observations"][10:11]))[0]

    action = planner.sample_action(obs, goal_z, step=0)
